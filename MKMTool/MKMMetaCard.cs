...
NOT STORED          reprint: [                  // Reprint entities for each similar product bundled by the metaproduct
    ...
    /// Gets the number of non-null attributes in this metacard.
    /// <returns>Number of attributes with other value than "". Only primary (synonyms don't count).</returns>
    public int GetNumberOfAttributes()
    {
      int count = 0;
      foreach (var att in data)
      {
        if (!synonyms.ContainsKey(att.Key) && att.Value != "")
          count++;
      }
      return count;
    }
