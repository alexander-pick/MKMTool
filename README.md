...
+ **LocName**: the name of the card in the language in which it is printed. Same rules as for **Name**.
...
+ **Altered**: whether it is altered or not. Valid values are (all case insensitive): "true" or "altered" for altered cards, "false" for non-altered cards, "null" or empty for when you do not care.
...
+ **MKMTool price**: the price assigned to the article by MKMTool, in € (not relevant for Price Update).
