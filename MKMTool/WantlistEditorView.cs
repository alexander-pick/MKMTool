...
namespace MKMTool
{
  public partial class WantlistEditorView : Form
  {
    ...
    // Reload the tables based on database each time the form is made visible for the corner case that
    // the database changed in between reload want lists in case user added a new one through MKM.
    ...
    private void wantsListBoxReload()
    {
      ...
      try
      {
        ...
        dv.Columns["enName"].SetOrdinal(0); // put the name and expansion at the beginning of the table
          ...
          if (child.Name == "item")
          {
            ...
            else // it is a metaproduct
            {
              dv.Rows[itemCounter]["enName"] = child["metaproduct"]["enName"].InnerText; // we don't care which child is the first, the name is the same for all
                                                                                         ...
                                                                                         // given that the current code refreshes this view every time an item is added to wantlist, it is not very practical
