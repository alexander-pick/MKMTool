...
+ Fix crashes in View Inventory when some of the expected columns are not there.
...
https://tomasjanak.github.io/MKMTool0.8.0.3.zip, compiled by [Tomas Janak](https://github.com/tomasjanak)). **IF YOU ARE UPDATING FROM A VERSION OLDER THAN 1.12.2020, delete mkminventory.csv file in your bin folder before starting MKMTool. But from now on, try to always keep this file even as you update to new versions, it will store additional data about cards in your stock so that MKMTool can use less API requests and be faster.**
...
+ **Update mode:** this relates to how the minimum prices from myStock.csv (description below this list) are used during price update. In the "Full update" mode, the new price is computed using the algorithm you set and at the end, the calculated price is compared to the minPrice from myStock.csv. If the minPrice is higher, it is used as the new price instead of the calculated one. Also, if the algorithm fails to compute a price (for example because of not enough similar items found), the minPrice will be compared to the current price and if it is below the minPrice, the new price will be set to minPrice. In the "Update only below minPrice" mode, the full update will run only for items that have current price below minPrice. No updates will be done for other items. Lastly, in the "Only ensure minPrice", the price calculation algorithm is not used at all: items with current price below minPrice will be set to minPrice, no updates will be done for other items.
